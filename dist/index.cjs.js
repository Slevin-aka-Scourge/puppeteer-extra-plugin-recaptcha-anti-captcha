...
class PuppeteerExtraPluginRecaptcha extends puppeteerExtraPlugin.PuppeteerExtraPlugin {
    ...
    async solvedAntigate(captchas){
        const providerID=ac.settings.taskId;
        ...
        const vendor=captchas._vendor;
        let token
        //this.debug("VENDOR",vendor,`\n${captchas.url}\n${captchas.sitekey}`);
        if(vendor==="hcaptcha"){
            token=await ac.solveHCaptchaProxyless(captchas.url,captchas.sitekey, '', enterprisePayload, isInvisible, isEnterprise)
        }else{
            if(captchas._type==='score'){
                    const action=captchas?.action?captchas.action:"";
                    this.debug("ACTION",action);
                    token=await ac.solveRecaptchaV3(captchas.url, captchas.sitekey, 0.7,action)
            }else{
                if(captchas.isEnterprise){
                    token= await ac.solveRecaptchaV2EnterpriseProxyless(captchas.url, captchas.sitekey,{"s":captchas.s})
                }else{
                    token = await ac.solveRecaptchaV2Proxyless(captchas.url, captchas.sitekey,captchas.isInvisible);
                }
            }
        }
        const responseAt=new Date()
        this.debug("CAPTCHA",token);
        const solution={
                _vendor: vendor,
                provider:"antigate",
                id: captchas.id,
                requestAt: requestAt,
                providerCaptchaId: providerID,
                text: token,
                responseAt: responseAt,
                hasSolution: true,
                duration: secondsBetweenDates(requestAt,responseAt)
              }
        return solution
    }
    async getRecaptchaSolutions(captchas, provider) {
        ...
        this.debug("PROVIDER",provider.id);
        this.debug("PROVIDER",captchas)
        if(provider.id==="antigate"){
            const solution={
                solutions: [],
                error: undefined
            }
            ac.setAPIKey(provider.token);
            ac.shutUp();
            ac.setSoftId(1193);
            this.debug("TOKEN",provider.token)
            this.debug("CAPTCHA",captchas);
            for(let i=0;i<captchas.length;i++){
                solution.solutions.push(this.solvedAntigate(captchas[i]))
            }
            solution.solutions=await Promise.all(solution.solutions)
